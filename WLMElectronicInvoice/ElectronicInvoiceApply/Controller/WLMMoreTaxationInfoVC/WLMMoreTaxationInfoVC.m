...
- (WLFormItemViewModel *)textFieldCellWithInfo:(NSDictionary *)info {
    ...
    @weakify(self);
    row.itemConfigBlock = ^(WLFormTextInputCell *cell, id value, NSIndexPath *indexPath) {  // 在cellForRow中调用了itemConfigBlock
    ...
    row.valueValidateBlock = ^NSDictionary *(id value) {    // WLForm的validateItems方法中会调用item的valueValidateBlock
        if ([value[kRightKey] length] || [value[kDispensable] boolValue]) {
            return itemValid();
        }
